...
public abstract class MathUtil {
    ...
    public static int convertAnRoundUpdMetersToKm(int meters){
        BigDecimal x = new BigDecimal(meters/1000.00);
        BigDecimal roundedDistance = x.round(new MathContext(MathUtil.countDigit(x), RoundingMode.UP));
        return roundedDistance.intValue();
    }

    public static int countDigit(BigDecimal x) {
    ...
    public static int getRoundUp(int seconds){
        int minutes = 0;
        if(seconds > 15) {
            if (seconds % 60 != 0) {
    ...
    public static final double sinh(double value) {
        ...
        return (exp(value) - exp(-value)) / 2f;
    }



    public static String splitDigit(int val){
        String res = "";
        if(val==0 || ((int)(Math.log10(val)+ 1))<2){
            res= "0"+val;
        }else{
            res = res+val;
        }
    ...
    private static final double ieee754_atan2 (double x, double y)
    {
        ...
        if(((ix|((lx|-lx)>>31))>0x7ff00000)||
            ...
            return x+y;
        if((hx-0x3ff00000|lx)==0) return ieee754_atan(y);   /* x=1.0 */
        ...
        else z=ieee754_atan(Math.abs(y/x));             /* safe to do y/x */
        switch (m) {
            case 0: return       z  ;   /* atan(+,+) */
            case 1: z = Double.longBitsToDouble(Double.doubleToLongBits(z) ^ 0x80000000); // __HI(z) ^= 0x80000000;
                return       z  ;   /* atan(-,+) */
